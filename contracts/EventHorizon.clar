;; EventHorizon - Decentralized Prediction Market Platform
;; A platform for creating and trading outcome shares for real-world events
...
(define-read-only (calculate-share-price (yes-pool uint) (no-pool uint) (bet-yes bool))
    (let (
        (total-pool (+ yes-pool no-pool))
        (target-pool (if bet-yes yes-pool no-pool))
    )
        (if (is-eq total-pool u0)
            u50 ;; Initial price of 0.5 STX per share
            (/ (* target-pool u100) total-pool)
        )
    )
)
...
;; Public functions
(define-public (create-market (title (string-ascii 256)) (description (string-ascii 1024)) (duration-blocks uint))
    (let (
        ...
        (asserts! (> (len validated-title) u0) ERR_INVALID_AMOUNT)
        (asserts! (> (len validated-description) u0) ERR_INVALID_AMOUNT)
...
(define-public (buy-shares (market-id uint) (bet-yes bool) (amount uint))
    (let (
        ...
        (new-yes-pool (if bet-yes (+ (get yes-pool market-data) net-amount) (get yes-pool market-data)))
        (new-no-pool (if bet-yes (get no-pool market-data) (+ (get no-pool market-data) net-amount)))
        (shares-to-add net-amount) ;; 1:1 ratio for simplicity
    )
        ...
        (map-set user-positions { market-id: market-id, user: tx-sender }
            (if bet-yes
                {
                    yes-shares: (+ (get yes-shares current-position) shares-to-add),
                ...
                }
                {
                    ...
                    no-shares: (+ (get no-shares current-position) shares-to-add),
                    total-invested: (+ (get total-invested current-position) amount)
                }
            )
        )
...
(define-public (claim-winnings (market-id uint))
    (let (
        ...
        (payout (if (> winning-pool u0) (/ (* winning-shares total-pool) winning-pool) u0))
    )
